...
### 推奨工具類
 ...
 - マスキングテープ  
### 必要ファイルのダウンロード
 - [re-aoharu-beta.uf2](Firm/re_aoharu_beta.uf2)  
   ファームウェアです。ハイフン以降がバージョン情報となり、Beta版のものが利用できます。  
...
## 既知の問題の確認
- ステムの揺れが大きいキースイッチや、寸法精度が極めて悪いキーキャップを使用すると、キーキャップ同士が干渉する可能性があります。
  - 通常の使用では問題ない可能性が高いですが、一部配置がオーバーラップするため、注意してください。
- LEDのカラーリングに異常が発生する場合があります。
  - F12と左Ctrlあたりで1つだけ変色するなどの事象が発生しています。
  - 再現が全く取れないので対策できていません。LEDは実装しない方がいいでしょう。
- LEDの消費電力が極めて大きくなっています。
  - 降圧用ダイオード保護のため、同梱の1N4007(黒く、太いダイオード)を基板-スイッチプレートの間に入るように実装してください。
  - ファームウェア側で最大光量を8割以下に制限しています。これにより最大光量が抑えられ、同時に発熱量も落ち着くようになっています。
...
最大輝度の調節を行う場合はconfig.h内の#define RGBLIGHT_LIMIT_VALの末尾の値を書き換えてください。初期値は200(78%)です。  
※輝度調節を行っただけのファームウェアの場合はサポートできます
